...
public class ElectricRequestDetailActivity extends AppCompatActivity {
    ...
    String JD_POSUDIAPPLYID;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.electric_request_detail_activity);
        ...
        if (!StringUtils.isEmpty(getIntent().getStringExtra("from")) && getIntent().getStringExtra("from").equals("waitdolist")) {//来自代办事项列表
            ...
            mResultlistBean = (ElectricRequestListBean.ResultBean.ResultlistBean) getIntent().getExtras().get("ResultlistBean");//来自首页 项目立项/项目月度计划
            status = mResultlistBean.getSTATUS();
            JD_POSUDIAPPLYID = mResultlistBean.getJD_POSUDIAPPLYID() + "";
    ...
    private void initEvent() {
        if (needGet) {
            ...
            tvRequestNo.setText("申请单号:" + mResultlistBean.getJD_POSUDIAPPLYID());
            ...
            tvBigType.setText("供配电大类:" + mResultlistBean.getTYPEDL());
            tvSmallType.setText("供配电小类:" + mResultlistBean.getTYPEXL());
            tvOwnDept.setText("所属单位:" + mResultlistBean.getSSDW());
            tvLocation.setText("使用地点:" + mResultlistBean.getSYDD());
            tvModel.setText("型号规格:" + mResultlistBean.getXH());
            tvEqStatue.setText("设备状态" + mResultlistBean.getASSETSTATUS());
            tvProductDate.setText("投产日期:" + mResultlistBean.getSXSH());
            tvZhibiao.setText("主要指标参数" + mResultlistBean.getSM());
            tvManufacturer.setText("制造商:" + mResultlistBean.getZZC());
            tvUsedDept.setText("使用单位:" + mResultlistBean.getSYDW());
            tvBianhao.setText("出厂编号:" + mResultlistBean.getJD_POSUDIAPPLYNUM());
            tvOriginValue.setText("原值:" + mResultlistBean.getTZJE());
            tvOwnDept.setText("使用单位:" + mResultlistBean.getSYDD());
    ...
    private void getDetail() {
         ...
         *   "option" : "read",
         *   "condition" : {
         *     "JD_POSUDIAPPLYID" : 101
         *   },
         *   "orderby" : "JD_POSUDIAPPLYID desc",
         *   "curpage" : 0,
         *   "showcount" : 1,
        ...
        object.put("orderby", "JD_POSUDIAPPLYID desc");
        JSONObject condition = new JSONObject();
        condition.put("JD_INFORMANAGEID", JD_POSUDIAPPLYID);
        ...
        OkhttpUtil.okHttpGet(url, map, headermap, new CallBackUtil.CallBackString() {
            ...
            public void onResponse(String response) {
                ...
                if (!response.isEmpty()) {
                    ...
                    JD_POSUDIAPPLYID = resultlistBean.getJD_POSUDIAPPLYID() + "";
                    tvRequestNo.setText("申请单号:" + resultlistBean.getJD_POSUDIAPPLYID());
                    tvRequestStatue.setText(mResultlistBean.getSTATUS());
                    tvSystemNo.setText("系统编号:" + resultlistBean.getASSETNUM());
                    tvEqName.setText("设备名称:" + resultlistBean.getNAME());
                    tvBigType.setText("供配电大类:" + resultlistBean.getTYPEDL());
                    tvSmallType.setText("供配电小类:" + resultlistBean.getTYPEXL());
                    tvOwnDept.setText("所属单位:" + resultlistBean.getSSDW());
                    tvLocation.setText("使用地点:" + resultlistBean.getSYDD());
                    tvModel.setText("型号规格:" + resultlistBean.getXH());
                    tvEqStatue.setText("设备状态" + resultlistBean.getASSETSTATUS());
                    tvProductDate.setText("投产日期:" + resultlistBean.getSXSH());
                    tvZhibiao.setText("主要指标参数" + resultlistBean.getSM());
                    tvManufacturer.setText("制造商:" + resultlistBean.getZZC());
                    tvUsedDept.setText("使用单位:" + resultlistBean.getSYDW());
                    tvBianhao.setText("出厂编号:" + resultlistBean.getJD_POSUDIAPPLYNUM());
                    tvOriginValue.setText("原值:" + resultlistBean.getTZJE());
                    tvOwnDept.setText("使用单位:" + resultlistBean.getSYDD());
    ...
    private void initView() {
        tvCommonTitle.setText("供配电台账增减申请");
    }
    ...
    private void start() {
        ...
        String request="<soap:Envelope xmlns:soap=\"http://www.w3.org/2003/05/soap-envelope\" xmlns:max=\"http://www.ibm.com/maximo\">\n" +
                ...
                "   </soap:Body>\n" +
                "</soap:Envelope>";
        request = String.format(request, JD_POSUDIAPPLYID, SharedPreferencesUtil.getString(this, "personId"));
        HashMap<String, String> headermap = new HashMap<>();
        headermap.put("Content-Type", "text/plan;charset=utf-8");
...
//        number_tv.setText("0/10");
    ...
    private void goApproval(int ifAgree, String opinion) {
         ...
         * 			<max:processname>GPDTZ</max:processname>
         * 			<max:mboName>JD_POSUDIAPPLY</max:mboName>
         * 			<max:keyValue>5</max:keyValue>
         * 			<max:key>JD_POSUDIAPPLYID</max:key>
        ...
        String request = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:max=\"http://www.ibm.com/maximo\">\n" +
                ...
                "\t</soapenv:Body>\n" +
                "</soapenv:Envelope>";
        request = String.format(request, JD_POSUDIAPPLYID, ifAgree, opinion, SharedPreferencesUtil.getString(this, "personId"));
        HashMap<String, String> headermap = new HashMap<>();
        headermap.put("Content-Type", "text/plan;charset=utf-8");
        headermap.put("SOAPAction", "urn:action");
